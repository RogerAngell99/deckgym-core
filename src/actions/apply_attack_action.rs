...
fn forecast_effect_attack_by_mechanic(
    ...
    match mechanic {
        ...
        Mechanic::DelayedAttackDamage { amount } => {
            delayed_attack_damage(attack.fixed_damage, *amount, attack.title.clone())
        }
        Mechanic::ReactiveAttackDamageNextTurn { amount } => {
            reactive_attack_damage_next_turn(attack.fixed_damage, *amount, attack.title.clone())
        }
...
fn switch_self_with_bench(state: &State, damage: u32, may_switch: bool) -> Outcomes {
    ...
    Outcomes::single(Box::new(
        move |_: &mut StdRng, state: &mut State, action: &Action| {
            ...
            // Switch effects resolve before final attack knockouts, so hit-and-run attackers
            // remain the attacking Pokemon even if they move to the Bench.
...
fn delayed_attack_damage(base_damage: u32, delayed_damage: u32, attack_name: String) -> Outcomes {
...
fn reactive_attack_damage_next_turn(
    base_damage: u32,
    reactive_damage: u32,
    attack_name: String,
) -> Outcomes {
