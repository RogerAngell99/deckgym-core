...
pub(crate) fn on_end_turn(player_ending_turn: usize, state: &mut State) {
    ...
    let delayed_attack_damages: Vec<(u32, usize, u64, String)> = state
        ...
        .map(|active| {
            active
                ...
                .iter()
                .filter_map(|(effect, _)| {
                    if let CardEffect::DelayedAttackDamage {
                        amount,
                        source_player,
                        source_play_id,
                        attack_name,
    ...
    for (amount, source_player, source_play_id, attack_name) in delayed_attack_damages {
        debug!(
            "Delayed attack damage: Applying {} damage to active Pokemon",
            amount
        );
        let source_idx = state
            .find_play_id(source_player, source_play_id)
            .unwrap_or(0);
        crate::actions::handle_damage(
            state,
            (source_player, source_idx),
